...
def update_labels(x1,x2,m1,m2,v1,v2,space):
    label_x1 = Label(root, text="x1=         ").place(x=20, y=250)
    label_x1 = Label(root, text="x1= " + str("%.2f" %x1)+"   ").place(x=20, y=250)
    label_x2 = Label(root, text="x2=         ").place(x=720, y=250)
    label_x2 = Label(root, text="x2= " + str("%.2f" %x2)+"   ").place(x=720, y=250)
...
#####################################################################
def set_clicked():
    global v1,v2,m1,m2,rad1,rad2,cir,cir2
    v1=var1.get()
    v2=var2.get()
    m1=var3.get()
    m2=var4.get()
...
def set_scales():
    ...
    v1_scale = Scale(root, from_=-300, to=300, orient='horizontal', bg='blue', variable=var1).place(x=0, y=300)
    v2_scale = Scale(root, from_=-300, to=300, orient='horizontal', bg='blue', variable=var2).place(x=700, y=300)
    m1_scale = Scale(root, from_=1, to=18, orient='horizontal', bg='blue', variable=var3).place(x=0, y=400)
    m2_scale = Scale(root, from_=1, to=18, orient='horizontal', bg='blue', variable=var4).place(x=700, y=400)

    set_button = Button(root, text="set", padx=40, bg='yellow', command=set_clicked).place(x=0, y=500)
    set_label=Label(root,text="Click set \nto update values")
    set_label.config(font=("Courier", 8))
    set_label.place(x=0,y=450)
######main program##################################
...
title.pack()
can=Canvas(root,width=800,height=600,background='black')
can.pack()
cir = draw_cir(x1, y, rad1, 'green')
cir2= draw_cir(x2, y, rad2, 'red')
...
ball1_label=Label(root,text="Ball 1",fg='green')
ball1_label.config(font=("Courier", 15))
ball1_label.place(x=20,y=200)

ball2_label=Label(root,text="Ball 2",fg='red')
ball2_label.config(font=("Courier", 15))
ball2_label.place(x=720,y=200)
#set labels
update_labels(x1, x2, m1, m2, v1, v2, space)
set_scales()
